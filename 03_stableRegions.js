...
var predStack = ee.Image(
  exportYears.map(function(year) {
    return ee.Image(ASSET_PREFIX + '02_RF1_2compCTRL_prediction_connectedFilter_' + year)
...
if (EXPORT_TO_DRIVE) {
  Export.table.toDrive({
    ...
    folder        : DRIVE_FOLDER,
    fileNamePrefix: '03_SumOnes_Histogram_rf1patchFiltered_compCtrl',
    fileFormat    : 'CSV'
  });
}
...
Export.image.toAsset({
  ...
  assetId    : ASSET_PREFIX + '03_RF2_stable_categories_rf1compCtrl',
  region     : roi,
  scale      : 10,
  maxPixels  : 1e13
});
...
if (EXPORT_TO_DRIVE) {
  Export.image.toDrive({
    ...
    folder        : DRIVE_FOLDER,
    fileNamePrefix: '03_RF2_stable_categories_rf1patchFiltered_rf1compCtrl',
    region        : roi,
    scale         : 10,
    maxPixels     : 1e13,
    fileFormat    : 'GeoTIFF'

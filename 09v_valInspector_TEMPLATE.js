// =============================================================================
// =============================================================================
// 09v: Independent-validation INSPECTOR (template)
// =============================================================================
// =============================================================================
//
// TEMPLATE — each validator copies this to `09v_valInspector_<name>.js` so the
// three of them work in parallel and independently.
//
...
// =============================================================================
var PXID      = 'px0001';     // ← pxid of the current row
var YEAR      = 2017;         // ← 2017 or 2025 (year being labelled)
var VALIDATOR = 'lican';      // ← your name (cosmetic)
...
Map.centerObject(contour, 17);
Map.setOptions('SATELLITE');
Map.addLayer(aoi,     {color: 'ffffff'}, '1km AOI', false);
Map.addLayer(contour, {color: 'FF0000'}, 'pixel contour ' + PXID, true);
...
Map.addLayer(merged, {bands: ['B4', 'B3', 'B2'], min: 0, max: 3000},
             YEAR + ' TrueColor NovDec', true);
Map.addLayer(merged.select('NDYI'), NDYI_VIS, YEAR + ' NDYI NovDec', false);
Map.addLayer(merged, {bands: ['B4_feb', 'B3_feb', 'B2_feb'], min: 0, max: 3000},
             YEAR + ' TrueColor Feb', false);

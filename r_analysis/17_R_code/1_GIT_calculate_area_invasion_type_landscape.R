...
pixeles_positivos <- global(stack > threshold, 
                            fun = "sum", 
                            na.rm = TRUE)

out_file2 <- file.path(
  results_dir,
  "positive_pixels_by_year.csv"
)
...
pixeles_positivos <- read.csv(out_file2)
...
roi <- vect(file.path(
  shapefile_dir,
  "16.1.2_study_area_border",
  "study_area_border.shp"
))
...
poblados <- vect(file.path(
  shapefile_dir,
  "16.1.5_Urban_roi",
  "4_poblados_study_area.shp"
))

rutas <- vect(file.path(
  shapefile_dir,
  "16.1.6_Roads_roi",
  "2_rutas_study_area.shp"
))

rios <- vect(file.path(
  shapefile_dir,
  "16.1.8_Rivers_roi",
  "2_rios_study_area.shp"
))
...
retama_x_clase <- readRDS(file.path(
     results_dir,
     "retama_x_clase.rds"
  ))
...
#  filename = file.path(
#    figure_dir,
#    "18.1_retama_temporal_trends_total.png"
#  ),
#  plot = fig1,
#  width = 8,
#  height = 4,
...
#  filename = file.path(
#    figure_dir,
#    "18.2_retama_temporal_trends_classes.png"
#  ),
#  plot = fig2,
#  width = 8,
#  height = 6,

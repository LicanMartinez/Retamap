...
# -----------------------------------------------------------------------------
# Step 2 of the NEAR-ONLY validation variant (2 strata, S3 dropped — see header
# of 09vNear_valGenerator.js for the rationale and the "conditional on the
# near-domain" caveat). Consumes the raw point CSV exported by
# 09vNear_valGenerator.js (Google Drive) and produces:
#
#   1. 09vNear_master_key.csv    — pxid + stratum + map labels (m2017,m2025) +
#                                  lon/lat + assigned validators. LOCAL ONLY,
#                                  never uploaded/published (it carries labels).
#   2. fc/09vNear_valPoints_fc.shp — BLIND FeatureCollection: pixel-contour
#      (+ .geojson)                polygons with ONLY `pxid`. Upload as GEE asset
#                                  `09vNear_valPoints_fc`. Centroid is derived in
#                                  GEE with .centroid().
#   3. sheets/09vNear_sheet_<val>.csv — one CSV per validator → one tab of the
...
#
# Design: 2 strata (S1 mapped retama, S2 near non-retama), N drawn by Olofsson
# (set in the generator, per-stratum SE). 3 validators. Validation load is NOT
# uniform across strata:
#   - S1 (mapped retama): PAIR-validated, 2 of 3 (round-robin pairs AB/BC/CA).
#     Drives the paper's retama UA/PA, so it keeps inter-observer agreement
#     (kappa) and disagreement-discarding as QC.
#   - S2 (near background): SINGLE-validated, 1 of 3 (round-robin A/B/C). Cuts
#     effort roughly in half for this stratum.
# pxid is a SHUFFLED sequential id so strata are not contiguous in the sheets.
#
# Outputs live in their OWN work dir (validation_independent_near/), separate
# from the 3-strata variant's validation_independent/, so the two point sets
# and asset names never collide.
# =============================================================================
...
WORK_DIR    <- file.path(PROJECT_DIR, "gee_scripts", "validation_independent_near")

# Place the Drive-downloaded raw CSV here (default name from the generator):
INPUT_CSV <- file.path(WORK_DIR, paste0("09vNear_valPoints_raw", RUN_SUFFIX, ".csv"))
...
stopifnot(all(raw$stratum %in% c(1, 2)))   # no S3 in this variant
cat(sprintf("Read %d points (S1=%d, S2=%d)\n",
            nrow(raw), sum(raw$stratum == 1), sum(raw$stratum == 2)))
...
#    - S2: single validation, 1 of 3 (round-robin A/B/C)
# =============================================================================
...
cat(sprintf("S1 pair-validated: %d points | S2 single-validated: %d points\n",
...
write.csv(raw[, c("pxid", "stratum", "m2017", "m2025", "lon", "lat", "validators")],
          file.path(WORK_DIR, "09vNear_master_key.csv"), row.names = FALSE, na = "")

# 5b. Blind FeatureCollection (pxid only) → asset upload -----------------------
fc_shp <- file.path(WORK_DIR, "fc", "09vNear_valPoints_fc.shp")
fc_gj  <- file.path(WORK_DIR, "fc", "09vNear_valPoints_fc.geojson")
...
for (v in VALIDATORS) {
  ...
  write.csv(sub, file.path(WORK_DIR, "sheets", paste0("09vNear_sheet_", v, ".csv")),
            row.names = FALSE, na = "")
}
...
cat(sprintf("\nDone. Outputs in: %s\n", WORK_DIR))
cat("  - 09vNear_master_key.csv  (LOCAL ONLY, has labels)\n")
cat("  - fc/09vNear_valPoints_fc.{shp,geojson}  → upload as GEE asset 09vNear_valPoints_fc\n")
cat(sprintf("  - sheets/09vNear_sheet_<%s>.csv  → import as tabs of one Google Sheet\n",
